...
const create = catchError(async (req, res) => {
  const { firstName, lastName, email, password, phone, frontBaseUrl } =
    req.body;
  ...
  await sendEmail({
    ...
    html: `
				...
				<br>
				${frontBaseUrl}/#/verify_email/${code}
			`,
  });
...
const login = catchError(async (req, res) => {
  ...
  if (!user.isVerified)
    return res.status(401).json({ message: "User not validate" });
...
const updatePasswordUser = catchError(async (req, res) => {
  ...
  return res.json({ message: "Nueva contraseña aplicada" });
});
